...
<div class="page-header">
    <h1 class="page-title"><i class="fas fa-file-contract me-2 text-primary"></i>Kontrak Kerja</h1>
...
<div class="card">
                ...
                @forelse($contracts as $contract)
                    <tr class="{{ $contract->is_expiring_soon ? 'table-warning' : '' }}">
                        <td>
                            ...
                            <small class="text-muted">{{ $contract->employee->department?->name }}</small>
                        </td>
                        ...
                        <td>
                            @if($contract->end_date && $contract->status === 'active')
                                ...
                                @if($days < 0)
                                    <span class="text-danger small">Kadaluarsa</span>
                                @elseif($days <= 30)
                                    <span class="badge bg-danger">{{ $days }} hari</span>
                                @else
                                    <span class="text-muted small">{{ $days }} hari</span>
                                @endif
                            @else
    ...
    @if($contracts->hasPages())<div class="card-footer">{{ $contracts->links() }}</div>@endif
</div>
@endsection
