...
<div class="page-header">
    <h1 class="page-title"><i class="fas fa-calendar-times me-2 text-primary"></i>Detail Pengajuan Cuti</h1>
    ...
    <div class="card-body">
        <div class="row g-3">
            ...
            <div class="col-md-6">
                ...
                <span class="badge bg-secondary">{{ $leave->leave_type_label }}</span>
            </div>
            <div class="col-md-6">
                <small class="text-muted d-block">Tanggal Mulai</small>
                <strong>{{ $leave->start_date->translatedFormat('d F Y') }}</strong>
            </div>
            <div class="col-md-6">
                <small class="text-muted d-block">Tanggal Selesai</small>
                <strong>{{ $leave->end_date->translatedFormat('d F Y') }}</strong>
            </div>
            <div class="col-md-6">
                <small class="text-muted d-block">Jumlah Hari</small>
                <strong>{{ $leave->days }} hari</strong>
            </div>
            <div class="col-md-6">
            ...
            @if($leave->notes)
                <div class="col-12">
                    <small class="text-muted d-block">Catatan Persetujuan</small>
                    <div class="p-3 bg-light rounded">{{ $leave->notes }}</div>
                </div>
            @endif
            @if($leave->approver)
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    ...
                    <strong>{{ $leave->approved_at?->format('d/m/Y H:i') }}</strong>
                </div>
            @endif
        </div>
                ...
                <div class="row g-2">
                    <div class="col-md-6">
                        <form action="{{ route('leaves.approve', $leave) }}" method="POST">
                            @csrf
                            <div class="mb-2">
                                <textarea name="notes" class="form-control form-control-sm" rows="2" placeholder="Catatan (opsional)"></textarea>
                            </div>
