...
<div class="d-flex align-items-center justify-content-between mb-3">
    <span class="text-muted">Detail Karyawan</span>
...
<div class="row g-3">
    <div class="col-lg-3">
        <div class="card mb-4">
            <div class="card-body text-center py-4">
                @if($employee->photo)
                    <img src="{{ Storage::url($employee->photo) }}" class="rounded-circle mb-3" style="width:100px;height:100px;object-fit:cover;border:3px solid #dee2e6;">
                @else
    ...
    <div class="col-lg-9">
        <div class="card mb-4">
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-4"><small class="text-muted d-block">Email</small><span>{{ $employee->email }}</span></div>
                    <div class="col-md-4"><small class="text-muted d-block">No. Telepon</small><span>{{ $employee->phone ?? '-' }}</span></div>
                    <div class="col-md-4"><small class="text-muted d-block">Jenis Kelamin</small><span>{{ $employee->gender_label ?? '-' }}</span></div>
                    <div class="col-md-4"><small class="text-muted d-block">Tanggal Lahir</small><span>{{ $employee->birth_date?->format('d/m/Y') ?? '-' }}</span></div>
                    <div class="col-md-4"><small class="text-muted d-block">NPWP</small><span>{{ $employee->npwp ?? '-' }}</span></div>
                    <div class="col-md-4"><small class="text-muted d-block">Kontak Darurat</small><span>{{ $employee->emergency_contact ?? '-' }}</span></div>
                    <div class="col-12"><small class="text-muted d-block">Alamat</small><span>{{ $employee->address ?? '-' }}</span></div>
                </div>
            </div>
        </div>
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-4"><small class="text-muted d-block">Tanggal Masuk</small><span>{{ $employee->hire_date?->format('d/m/Y') ?? '-' }}</span></div>
                    <div class="col-md-4"><small class="text-muted d-block">Gaji Pokok (Jabatan)</small><span>Rp {{ number_format($employee->position?->basic_salary ?? 0, 0, ',', '.') }}</span></div>
                    <div class="col-md-4"><small class="text-muted d-block">Bank / No. Rekening</small><span>{{ $employee->bank_name ?? '-' }} / {{ $employee->bank_account ?? '-' }}</span></div>
                </div>
            </div>
        </div>
                            ...
                            <tbody>
                                @forelse($employee->documents as $doc)
                                    <tr>
                                        ...
                                        <td><a href="{{ route('documents.download', $doc) }}" class="btn btn-sm btn-outline-primary"><i class="fas fa-download"></i></a></td>
                                    </tr>
                                @empty
