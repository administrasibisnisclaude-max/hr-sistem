...
<div class="row g-3">
    <div class="col-lg-8">
        <div class="card mb-4">
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-6"><small class="text-muted d-block">Karyawan</small><strong>{{ $payroll->employee->name }}</strong><br><small class="text-muted">{{ $payroll->employee->nik }} - {{ $payroll->employee->position?->name }}</small></div>
                    <div class="col-md-6"><small class="text-muted d-block">Departemen</small><strong>{{ $payroll->employee->department?->name }}</strong></div>
                    <div class="col-md-6"><small class="text-muted d-block">Periode</small><strong>{{ $payroll->period_label }}</strong></div>
                    <div class="col-md-6"><small class="text-muted d-block">Status</small>
        ...
        <div class="card mb-4">
            ...
            <div class="table-responsive">
                <table class="table mb-0">
                    ...
                    <tbody>
                        <tr><td>Gaji Pokok</td><td><span class="badge bg-primary">Gaji Pokok</span></td><td class="text-end fw-semibold">Rp {{ number_format($payroll->basic_salary, 0, ',', '.') }}</td></tr>
                        @if($payroll->overtime_pay > 0)
                            <tr><td>Uang Lembur</td><td><span class="badge bg-info">Lembur</span></td><td class="text-end text-success">Rp {{ number_format($payroll->overtime_pay, 0, ',', '.') }}</td></tr>
                        @endif
                    ...
                    <tfoot>
                        <tr class="table-light"><td colspan="2" class="fw-bold">Gaji Kotor</td><td class="text-end fw-bold">Rp {{ number_format($payroll->gross_salary, 0, ',', '.') }}</td></tr>
                        <tr class="table-primary"><td colspan="2" class="fw-bold fs-5">Gaji Bersih</td><td class="text-end fw-bold fs-5 text-primary">Rp {{ number_format($payroll->net_salary, 0, ',', '.') }}</td></tr>
                    </tfoot>
                </table>
            </div>
        </div>
