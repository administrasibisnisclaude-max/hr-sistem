    ...
    <div class="row g-3">
                ...
                <div class="card-body">
                    <div id="itemsContainer">
                        <div class="row g-2 mb-2 item-row">
                            ...
                            <div class="col-md-3">
                                <input type="number" name="items[0][amount]" class="form-control form-control-sm item-amount" placeholder="Jumlah" min="0">
                            </div>
                            <div class="col-md-1">
        ...
        <div class="col-lg-4">
            <div class="card sticky-top" style="top:80px;">
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between mb-2"><span>Gaji Pokok</span><span class="fw-semibold" id="sumBasic">Rp 0</span></div>
                    <div class="d-flex justify-content-between mb-2 text-success"><span>+ Tunjangan & Bonus</span><span id="sumAllowance">Rp 0</span></div>
                    <div class="d-flex justify-content-between mb-2 text-success"><span>+ Lembur</span><span id="sumOvertime">Rp 0</span></div>
                    <div class="d-flex justify-content-between mb-2 text-danger"><span>- Potongan</span><span id="sumDeduction">Rp 0</span></div>
                    <hr>
                    <div class="d-flex justify-content-between fw-bold fs-5"><span>Gaji Bersih</span><span class="text-primary" id="sumNet">Rp 0</span></div>
                </div>
...
document.getElementById('addItem').addEventListener('click', function() {
    ...
    row.innerHTML = `
        <div class="col-md-3"><select name="items[${itemCount}][type]" class="form-select form-select-sm"><option value="allowance">Tunjangan</option><option value="deduction">Potongan</option><option value="bonus">Bonus</option></select></div>
        <div class="col-md-5"><input type="text" name="items[${itemCount}][name]" class="form-control form-control-sm" placeholder="Nama komponen"></div>
        <div class="col-md-3"><input type="number" name="items[${itemCount}][amount]" class="form-control form-control-sm item-amount" placeholder="Jumlah" min="0"></div>
        <div class="col-md-1"><button type="button" class="btn btn-sm btn-outline-danger remove-item"><i class="fas fa-times"></i></button></div>
    `;
...
function bindEvents() {
    document.querySelectorAll('.remove-item').forEach(btn => { btn.onclick = function() { this.closest('.item-row').remove(); updateSummary(); }; });
    document.querySelectorAll('.item-amount, select[name*="[type]"]').forEach(el => { el.onchange = updateSummary; });
}
